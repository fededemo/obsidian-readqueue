...
| B-602 | ✅ **DONE** — `src/x-sync.ts`: lector del SQLite de birdclaw + dedupe vía `url-canon` + router `read`/`watch`/`reference` (taxonomía v2, §12 del plan). **Bloqueante de E1 y E2** | TODO | builder | B-601 ✅, B-606 ✅ | Comando "Sincronizar bookmarks de X"; tests con fixtures reales de los 450 |
...
| B-738 | Los tweets **"Link al paper: <t.co>"** no se pueden clasificar por texto (15 chars propios) — quedan sin `shelfLife`/`tldr`. El valor está en el hilo padre + el PDF. Resolver vía `birdclaw research` (expande el hilo → resumen + link), como diseñó F7 §11.3 | TODO | builder | B-603b | Los papers entran con contexto, no como "Link al paper:" pelado |
...
| B-737 | Aplicar el estándar como gate del gardener programado (B-712): toda nota-concepto nueva pasa el checklist de 8 puntos antes de escribirse | TODO | builder | B-736, B-712 | El pase semanal no puede introducir notas que no cumplan |
