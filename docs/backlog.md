...
| B-327 | 🐛 **BUG GRAVE — highlights de Kindle cuadruplicados.** Medido 2026-08-01: 2397 highlights en `Inbox/Kindle/`, solo **774 únicos (67% duplicados)**, **34 de 34 libros afectados**, patrón exacto **4×** (229→58, 200→50, 144→36, 136→34, 128→32). El merge incremental (MX12 `kindle-merge.ts`) re-agregó todo en cada sync en vez de deduplicar. Impacto: `highlightCount` inflado 4×, el resurfacing diario mostraría repetidos, y contamina las conexiones de F6 | TODO | builder | — | Root cause en `kindle-merge.ts` + test de regresión; script de limpieza idempotente sobre los 34 (con git como undo) |
