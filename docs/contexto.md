<!-- Cuerpo anterior del entrypoint. No se @importa. El entrypoint es CLAUDE.md. -->
...
7. **Cuando falta un dato, sospechar del parseo antes que de la fuente.** Las imágenes de X nunca se perdieron: `media_json` traía la URL al lado del `type`, y el sync mapeaba solo el `type` porque era lo único que necesitaba para decidir *watch* vs *read* (B-744). 345 notas quedaron ilegibles por un `.map()`. **Corolario**: cuando dos caminos de entrada producen el mismo tipo de nota —el intake de una URL pegada a mano y el sync de bookmarks— arreglar uno no arregla el otro. `tweetToArticle` embebía las fotos desde siempre; el código estaba escrito y a nadie se le ocurrió portarlo. Antes de escribir la solución, buscarla en el gemelo. **Y si el gemelo tampoco lo tiene, mirar el JSON que ya pedimos**: B-747, los X Articles. El HTML de `x.com/i/article/…` es un 404 vacío y B-740 concluyó "hace falta sesión autenticada". FxTwitter ya devolvía `tweet.article` (título + Draft.js) en la misma request que usábamos para el texto truncado; intake y sync tiraban el campo. El cuerpo nunca se perdió.
8. **Un pipeline no está terminado hasta que alguien lo dispara solo.** El sync de X pasó 14 días sin traer nada y el dry-run informaba «0 pendientes» con toda razón: el escalón que faltaba correr era el de más arriba (`birdclaw sync`, de X al SQLite), no el que tenía tests (B-745). Cuando el diagnóstico dice que todo está al día, preguntar **al día contra qué** — si la fuente de comparación también está congelada, la respuesta es circular. Y el corolario del arreglo: un job automático que falla en silencio reproduce el problema que vino a resolver, así que el wrapper avisa por notificación de macOS y deja `sync-x.state` con una línea por corrida.

**Jobs programados** (`scripts/launchd/`, log en `~/Library/Logs/readqueue/`): **sync de X** diario 21:00 — *instalado y corriendo*; **gardener** semanal domingo 20:00 — *escrito, sin instalar* (escribe en la vault, espera OK de Fede). Los dos comparten `lib/resolve-node.sh`, que busca un node que **ande**, no uno que exista.

**Pendiente de Fede** (ningún agente lo puede hacer): aplicar las conexiones a `Concepts/` (B-731) y los conceptos latentes (B-741), instalar el LaunchAgent del gardener, el primer sync real de Kindle (F5.0, B-321), el spike del Cloud Reader (B-324) y `xcode-select --install` (B-607 — `/opt/homebrew/bin/node` está roto; los jobs de launchd ya lo esquivan, pero cualquier `npm run` a mano sigue muriendo con `Abort trap: 6`).
